 ...
 * Throws a specific MISSING_API_KEY error if neither is set.
 */
...
router.post('/chat', requireAuth, async (req, res) => {
  try {
    const { messages, model = 'gpt-4o-mini', temperature = 0.7, max_tokens, response_format, system } = req.body;
    ...
    res.json({ content: completion.choices[0].message.content, usage: completion.usage });
  } catch (err) {
