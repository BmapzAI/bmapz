...
export default function SendMessageModal({ template, onClose }) {
  ...
  const handleSend = async () => {
    if (!selectedListId) { toast.error('Select a lead list'); return; }
    if (!customContent.trim()) { toast.error('Message content is required'); return; }
    if (leadsInList.length === 0) { toast.error('No leads in selected list'); return; }
    ...
    setSending(false);
    toast.success(`Sent to ${successCount}/${leadsInList.length} leads`);
    onClose();
  };
        ...
        <div className="space-y-4 py-2">
          ...
          <div>
            <label className="text-gray-400 text-sm mb-1.5 block flex items-center gap-1">
              <Users size={14} /> Select Lead List
            </label>
            ...
            {selectedListId && (
              <p className="text-gray-500 text-xs mt-1">
                {leadsInList.length} leads will receive this message
              </p>
            )}
          </div>
          ...
          <div>
            <label className="text-gray-400 text-sm mb-1.5 block">Message Content</label>
            {template?.subject && (
            ...
            />
            <p className="text-gray-600 text-xs mt-1">Variables like [First Name], [Company Name] will be personalized per lead</p>
          </div>
          ...
          {sending && (
            <div className="p-3 rounded-xl bg-[#38b6ff]/10 border border-[#38b6ff]/20">
              <div className="flex items-center justify-between text-sm mb-2">
                <span className="text-[#38b6ff]">Sending...</span>
          ...
          <div className="flex gap-3 pt-2">
            <Button onClick={handleSend} disabled={sending || !selectedListId}
              ...
              <Send size={16} />
              {sending ? `Sending... ${sent}/${total}` : `Send to ${leadsInList.length} leads`}
            </Button>
