...
export default function AdsStrategyForm({ form, setForm, company, isGenerating, onGenerate, onSave, isSaving, onOpenGuide, hasStrategy }) {
  ...
  const OBJECTIVES = [
    ...
    { value: 'direct sales', label: t('directSales') },
    { value: 'brand awareness', label: t('awareness') },
    { value: 'app installs', label: t('appInstalls') },
    { value: 'retargeting', label: t('retargeting') },
    { value: 'scaling', label: t('scaling') },
  ];
    ...
    <div className="rounded-2xl bg-white/5 border border-white/10 p-6 space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="text-white font-semibold flex items-center gap-2">
          <Sparkles size={18} className="text-[#38b6ff]" /> {t('strategyBuilder')}
        </h3>
        {company && (
          <span className="text-xs px-2 py-0.5 rounded-full bg-green-500/20 text-green-400 flex items-center gap-1">
            <Check size={10} /> {t('autoFilled')}
          </span>
        )}
      </div>
      <div>
        <label className="text-gray-400 text-sm">{t('campaignObjectiveLabel')}</label>
        <Select value={form.objective} onValueChange={(v) => setForm(p => ({ ...p, objective: v }))}>
          <SelectTrigger className="mt-1.5 bg-black/30 border-white/10 text-white"><SelectValue placeholder={t('selectObjectivePlaceholder')} /></SelectTrigger>
      ...
      <div>
        <label className="text-gray-400 text-sm">{t('platformLabel')}</label>
        <Select value={form.platform} onValueChange={(v) => setForm(p => ({ ...p, platform: v }))}>
          <SelectTrigger className="mt-1.5 bg-black/30 border-white/10 text-white"><SelectValue placeholder={t('selectPlatformPlaceholder')} /></SelectTrigger>
          <SelectContent className="bg-[#1a1a1a] border-white/10">
      ...
      <div>
        <label className="text-gray-400 text-sm">{t('monthlyBudgetLabel')}</label>
      ...
      <div>
        <label className="text-gray-400 text-sm">{t('productService')}</label>
      ...
      <div>
        <label className="text-gray-400 text-sm">{t('targetAudienceLabel')}</label>
      ...
      <div>
        <label className="text-gray-400 text-sm">{t('mainDifferentiator')}</label>
        <Input value={form.differentiator} onChange={(e) => setForm(p => ({ ...p, differentiator: e.target.value }))}
          placeholder={t('whatMakesDifferent')} className="mt-1.5 bg-black/30 border-white/10 text-white" />
      </div>
      <div>
        <AIContextField
